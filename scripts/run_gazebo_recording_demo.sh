...
WORLD_FILE="$ROOT_DIR/out/gazebo/${DEMO_NAME}_gazebo_recording.sdf"
...
POSE_STREAM_FILE="$ROOT_DIR/out/gazebo/${DEMO_NAME}_recording_pose_stream.csv"
...
echo "[gazebo-recording] exporting Gazebo world for ${DEMO_NAME}"
...
if [ ! -f "$QP_TRAJECTORY_FILE" ] || [ "$QP_TRAJECTORY_FILE" -ot "$CASE_FILE" ]; then
  echo "[gazebo-recording] exporting paper-QP trajectory cache at visual altitude 1.65m"
...
if [ ! -f "$RECORDING_TRAJECTORY_FILE" ] || [ "$RECORDING_TRAJECTORY_FILE" -ot "$QP_TRAJECTORY_FILE" ]; then
  echo "[gazebo-recording] freezing cached QP trajectory into uniform recording path"
  python3 "$ROOT_DIR/tools/freeze_uniform_recording_trajectory.py" \
    --input "$QP_TRAJECTORY_FILE" \
    --out "$RECORDING_TRAJECTORY_FILE" \
    --frames 960
else
  echo "[gazebo-recording] reusing frozen uniform recording path: $RECORDING_TRAJECTORY_FILE"
fi

echo "[gazebo-recording] exporting in-process Gazebo pose stream"
...
echo "[gazebo-recording] building Gazebo in-process replay plugin"
...
if "payload_demo::PayloadPoseReplaySystem" not in text:
    text = text.replace("  </world>", plugin_block + "  </world>", 1)
world.write_text(text, encoding="utf-8")
PY

echo "[gazebo-recording] opening $WORLD_FILE"
echo "[gazebo-recording] replay uses fixed uniform trajectory inside Gazebo; no QP or per-frame service calls"
